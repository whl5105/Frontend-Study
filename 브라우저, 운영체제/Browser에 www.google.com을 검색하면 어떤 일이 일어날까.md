...
7. 서버가 HTTP 응답을 보냅니다.
8. [브라우저는 응답받은 데이터를 파싱하여 화면에 렌더과정을 통해 화면에 웹페이지 내용물 출력합니다.](./%EB%B8%8C%EB%9D%BC%EC%9A%B0%EC%A0%80%EC%9D%98%20%EB%A0%8C%EB%8D%94%EB%A7%81%20%EB%90%98%EB%8A%94%20%EA%B3%BC%EC%A0%95.md)
...
컴퓨터(클라이언트)와 서버 간에 데이터 패킷을 전송하려면 TCP 연결을 설정하는 것이 중요합니다. 이 연결은 다음 프로세스를 사용하여 설정됩니다.[***TCP/IP three-way handshake***](../%EB%84%A4%ED%8A%B8%EC%9B%8C%ED%81%AC%2C%20%EB%B0%B1%EC%97%94%EB%93%9C/3-way-handshake%2C4-way-handshake.md). 이것은 클라이언트와 서버가 SYN(동기화) 및 ACK(확인) 메시지를 교환하여 연결을 설정하는 3단계 프로세스입니다.
...
### 8. 브라우저는 응답받은 데이터를 파싱하여 화면에 렌더과정을 통해 화면에 웹페이지 내용물 출력합니다.
